...
#define FUSE_EXTFUSE_RUNTIME_CONTRACT \
	"extfuse-contiguous-write-20260906"
...
static void fuse_free_inode(struct inode *inode)
{
	...
	if (IS_ENABLED(CONFIG_FUSE_PASSTHROUGH)) {
		...
		WARN_ON(fi->extfuse_wbcache_release_pending);
		fuse_backing_put(fb);
		if (wbcache_fb && wbcache_fb != fb)
			fuse_backing_put(wbcache_fb);
	}
			       ...
			       int error)
{
	...
	if (error || arg->major != FUSE_KERNEL_VERSION)
		...
		if (arg->minor >= 6) {
			...
			if (flags & FUSE_EXTFUSE_PASSTHROUGH_COHERENCE_V2) {
				if (!(flags & FUSE_EXTFUSE_PASSTHROUGH_COHERENCE) ||
