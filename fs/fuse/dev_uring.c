...
static struct fuse_ring_queue *
fuse_uring_background_queue(struct fuse_ring *ring, struct fuse_req *req)
{
	u64 stream_id = req->in.h.nodeid;
	unsigned int seq;
	unsigned int qid;
	...
	if ((req->in.h.opcode == FUSE_READ ||
	    ...
	    req->args->extfuse_file && req->args->extfuse_file->fh)
		stream_id = req->args->extfuse_file->fh;

	if (stream_id) {
		qid = hash_64(stream_id, 32) % ring->nr_queues;
	} else {
		seq = (unsigned int)atomic_fetch_inc_relaxed(&ring->bg_queue_seq);
		qid = seq % ring->nr_queues;
	}
...
bool fuse_uring_queue_bq_req(struct fuse_req *req)
{
	...
	queue = fuse_uring_background_queue(ring, req);
	if (!queue)
		return false;
